# T-200: Romania — Spectrum International College (Cluj-Napoca) world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:46+00:00
...
Produce a world-class, deeply researched high school profile for **Spectrum International College (Cluj-Napoca)** in **Cluj-Napoca, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier B per feeder map (school_id: sch-ro-cluj-spectrum)

---

## Allowed write paths

- `tickets/T-167-romania-sch-ro-cluj-spectrum-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-cluj-spectrum/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-cluj-spectrum` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-cluj-spectrum` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Spectrum International College (Cluj-Napoca)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
**What changed:** Added full Spectrum Cluj profile using the global template, refreshed school rows in both Romania CSVs with sources and affordability notes, and linked the new profile from the country schools index.
