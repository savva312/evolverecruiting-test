# T-192: Romania — Bucharest-Beirut International School world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:46Z
...
Produce a world-class, deeply researched high school profile for **Bucharest-Beirut International School** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-bbis)

---

## Allowed write paths

- `tickets/T-159-romania-sch-ro-bucharest-bbis-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-bbis/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-bbis` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-bbis` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Bucharest-Beirut International School`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
**What changed:** Created BBIS profile using the global template and updated both Romania school datasets (high-potential CSV and schools.csv) with aligned fields and sources dated 2025-12-20.
