# T-193: Romania — Genesis College world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:39:10+00:00
...
Produce a world-class, deeply researched high school profile for **Genesis College** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-genesis)

---

## Allowed write paths

- `tickets/T-160-romania-sch-ro-bucharest-genesis-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-genesis/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-genesis` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-genesis` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Genesis College`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
What changed: Created full Genesis College profile, aligned CSV rows, and marked ticket complete on 2025-12-20.
