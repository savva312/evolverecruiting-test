# T-201: Romania — Spectrum International College (Iași) world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20 22:51 UTC
...
Produce a world-class, deeply researched high school profile for **Spectrum International College (Iași)** in **Iași, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier B per feeder map (school_id: sch-ro-iasi-spectrum)

---

## Allowed write paths

- `tickets/T-168-romania-sch-ro-iasi-spectrum-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-iasi-spectrum/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-iasi-spectrum` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-iasi-spectrum` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Spectrum International College (Iași)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
**What changed:** Added Spectrum Iași profile (snapshot, contacts, fees, outreach plan), updated Romania school datasets for sch-ro-iasi-spectrum, and linked the profile in the Romania schools README.
