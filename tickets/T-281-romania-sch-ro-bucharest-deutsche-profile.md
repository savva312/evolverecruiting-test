# T-281: Romania — Deutsche Schule Bukarest world-class profile build
...
What changed: Created DSBU profile, refreshed Romania school datasets/README, captured sources.

---

## Goal

Produce a world-class, deeply researched high school profile for **Deutsche Schule Bukarest** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-deutsche)

---

## Allowed write paths

- `tickets/T-249-romania-sch-ro-bucharest-deutsche-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-deutsche/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-deutsche` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-deutsche` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Deutsche Schule Bukarest`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
