# T-203: Romania — EVISS High School (Valea Adâncă) world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:59+00:00
Last-updated: 2025-12-20

What changed (2025-12-20): Drafted EVISS profile, refreshed Romania school datasets for sch-ro-iasi-eviss, and linked the profile in the Romania schools index; sources aligned to the 2025-12-20 feeder map and school website.

---

## Goal

Produce a world-class, deeply researched high school profile for **EVISS High School (Valea Adâncă)** in **Iași metro, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier B per feeder map (school_id: sch-ro-iasi-eviss)

---

## Allowed write paths

- `tickets/T-170-romania-sch-ro-iasi-eviss-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-iasi-eviss/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-iasi-eviss` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-iasi-eviss` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `EVISS High School (Valea Adâncă)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
