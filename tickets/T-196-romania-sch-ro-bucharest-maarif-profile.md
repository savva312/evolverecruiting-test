# T-196: Romania — Maarif International School of Bucharest (candidate) world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:31+00:00
...
Produce a world-class, deeply researched high school profile for **Maarif International School of Bucharest (candidate)** in **Voluntari (Bucharest metro), Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-maarif)

---

## Allowed write paths

- `tickets/T-163-romania-sch-ro-bucharest-maarif-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-maarif/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-maarif` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-maarif` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Maarif International School of Bucharest (candidate)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- What changed (2025-12-20): Added candidate profile for Maarif Bucharest, synced Tier A data rows (schools.csv + high-potential CSV), and documented verification gaps (authorization, fees, leadership).
