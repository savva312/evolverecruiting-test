# T-246: Serbia — Košarkaška gimnazija “Koledž” world-class profile build
...
Produce a world-class, deeply researched high school profile for **Košarkaška gimnazija “Koledž”** in **Belgrade, Serbia**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Affordability tier High per Serbia pipeline index (school_id: koledz-basketball-gymnasium)

---

## Allowed write paths

- `tickets/T-213-serbia-koledz-basketball-gymnasium-profile.md`
...
- `countries/serbia/entities/schools/profiles/belgrade/koledz-basketball-gymnasium/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/serbia/data/entities/schools.csv` row for `koledz-basketball-gymnasium` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/serbia/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Košarkaška gimnazija “Koledž”`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- Current run: focusing on the school profile file only (index/README/CSV updates will follow separately to avoid collisions).

---

## Closure notes

- Closed and superseded by T-262.
