# T-186: Romania — Lycée Français Anna de Noailles world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:06Z
Last-updated: 2025-12-20
What changed: Added full AEFE lycée profile plus aligned rows in schools.csv and high-potential feeder CSV.

---

## Goal

Produce a world-class, deeply researched high school profile for **Lycée Français Anna de Noailles** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-lycee-francais)

---

## Allowed write paths

- `tickets/T-153-romania-sch-ro-bucharest-lycee-francais-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-lycee-francais/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-lycee-francais` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-lycee-francais` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Lycée Français Anna de Noailles`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
