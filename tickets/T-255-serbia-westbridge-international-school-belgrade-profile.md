# T-255: Serbia — Westbridge International School (Belgrade) world-class profile build
...
Produce a world-class, deeply researched high school profile for **Westbridge International School (Belgrade)** in **Belgrade, Serbia**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Affordability tier Medium-High per Serbia pipeline index (school_id: westbridge-international-school-belgrade)

---

## Allowed write paths

- `tickets/T-222-serbia-westbridge-international-school-belgrade-profile.md`
...
- `countries/serbia/entities/schools/profiles/belgrade/westbridge-international-school-belgrade/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/serbia/data/entities/schools.csv` row for `westbridge-international-school-belgrade` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/serbia/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Westbridge International School (Belgrade)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- 2025-12-21: Drafted Westbridge profile (Belgrade) per template; CSV/index updates intentionally deferred to avoid conflicts with parallel tickets.

---

## Closure notes

- Closed and superseded by T-270.
