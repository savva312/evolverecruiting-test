# T-247: Serbia — École française de Belgrade world-class profile build
...
Produce a world-class, deeply researched high school profile for **École française de Belgrade** in **Belgrade, Serbia**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Affordability tier High per Serbia pipeline index (school_id: ecole-francaise-de-belgrade)

---

## Allowed write paths

- `tickets/T-214-serbia-ecole-francaise-de-belgrade-profile.md`
...
- `countries/serbia/entities/schools/profiles/belgrade/ecole-francaise-de-belgrade/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/serbia/data/entities/schools.csv` row for `ecole-francaise-de-belgrade` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/serbia/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `École française de Belgrade`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- Per request, this run updates only the school profile (no index/README/CSV changes) to avoid conflicts with parallel tickets.

---

## Closure notes

- Closed and superseded by T-263.
