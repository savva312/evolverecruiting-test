# T-190: Romania — Verita International School world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:37:43Z
...
Produce a world-class, deeply researched high school profile for **Verita International School** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-verita)

---

## Allowed write paths

- `tickets/T-157-romania-sch-ro-bucharest-verita-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-verita/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-verita` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-verita` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Verita International School`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
### What changed
- Added full IB-aligned profile for sch-ro-bucharest-verita with outreach plan and sourcing notes.
- Updated Romania high-potential CSV, core schools.csv entry, and Bucharest schools index link for consistency.
