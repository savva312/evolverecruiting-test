# T-220: Lebanon — Lycée Français International Marcel Pagnol (LFIMP) world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:41:48+00:00
...
Produce a world-class, deeply researched high school profile for **Lycée Français International Marcel Pagnol (LFIMP)** in **Adonis–Zouk Mosbeh, Lebanon**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Development fund $4,300 (2025-26); Tier: Medium–High; source https://www.lfimp.education/frais-de-scolarite/?utm_source=openai (school_id: lb-adonis-lycee-marcel-pagnol)

---

## Allowed write paths

- `tickets/T-187-lebanon-lb-adonis-lycee-marcel-pagnol-profile.md`
...
- `countries/lebanon/entities/schools/profiles/lb-adonis-lycee-marcel-pagnol/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/lebanon/data/entities/schools.csv` row for `lb-adonis-lycee-marcel-pagnol` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/lebanon/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Lycée Français International Marcel Pagnol (LFIMP)`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
## What changed
- Added full LFIMP profile using the global high school template with affordability, outreach plan, and source links.
- Updated `countries/lebanon/data/entities/schools.csv` for LFIMP (affordability notes, contact, verification date).
