# T-197: Romania — The Royal School in Transylvania world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:40:00Z
...
Produce a world-class, deeply researched high school profile for **The Royal School in Transylvania** in **Cluj-Napoca, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier B per feeder map (school_id: sch-ro-cluj-royal)

---

## Allowed write paths

- `tickets/T-164-romania-sch-ro-cluj-royal-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-cluj-royal/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-cluj-royal` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-cluj-royal` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `The Royal School in Transylvania`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
**What changed (2025-12-20):** Created Royal School in Transylvania profile, updated high-potential CSV and main schools.csv entry, and linked the profile from the Romania schools index.
