# T-248: Serbia — Deutsche Schule Belgrad world-class profile build
...
Produce a world-class, deeply researched high school profile for **Deutsche Schule Belgrad** in **Belgrade, Serbia**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Affordability tier High per Serbia pipeline index (school_id: deutsche-schule-belgrad)

---

## Allowed write paths

- `tickets/T-215-serbia-deutsche-schule-belgrad-profile.md`
...
- `countries/serbia/entities/schools/profiles/belgrade/deutsche-schule-belgrad/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/serbia/data/entities/schools.csv` row for `deutsche-schule-belgrad` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/serbia/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Deutsche Schule Belgrad`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- Interim note (2025-12-21): Profile created; CSV/index updates intentionally deferred to avoid collisions with parallel tickets.

---

## Closure notes

- Closed and superseded by T-264.
