# T-185: Romania — European School of Bucharest world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:36:24Z
...
Produce a world-class, deeply researched high school profile for **European School of Bucharest** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-seb)

---

## Allowed write paths

- `tickets/T-152-romania-sch-ro-bucharest-seb-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-seb/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-seb` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-seb` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `European School of Bucharest`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- Created full SEB profile (bilingual/IB DP, fees, outreach plan, sources).
- Updated Romania high-potential CSV and schools.csv rows for sch-ro-bucharest-seb with tuition signals and contacts.
- Linked the new profile from the Romania schools README.
