# T-284: Nigeria — Lycée Français Louis Pasteur world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-21T10:16:04Z
...
- `tickets/T-252-nigeria-ng-lagos-lycee-francais-profile.md`
...
- [x] Data rows in `countries/nigeria/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Lycée Français Louis Pasteur`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- Previous note requested profile-only changes; this execution also synced the CSV row per acceptance criteria while avoiding other index edits.

## What changed

- Refreshed the Lycée Français Louis Pasteur profile with completed snapshot, affordability rationale, outreach plan, and clearly labeled gaps for contacts/calendar/outcomes.
- Synced `countries/nigeria/data/entities/schools.csv` for `ng-lagos-lycee-francais` with Tier1 affordability evidence, program-fit tags (including Medicine), and updated `last_verified`.
