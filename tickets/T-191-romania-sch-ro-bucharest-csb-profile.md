# T-191: Romania — Cambridge School of Bucharest world-class profile build
...
Claimed-by: work
Claimed-at: 2025-12-20T22:36:58Z
...
Produce a world-class, deeply researched high school profile for **Cambridge School of Bucharest** in **Voluntari (Bucharest metro), Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-csb)

---

## Allowed write paths

- `tickets/T-158-romania-sch-ro-bucharest-csb-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-csb/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-csb` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-csb` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [x] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Cambridge School of Bucharest`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- What changed (2025-12-20): Built CSB profile, synced entries in schools.csv and romania-high-potential-feeder-high-schools.csv, and linked the new profile from the Romania schools index.
