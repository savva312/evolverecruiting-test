# T-279: Romania — Avenor College world-class profile build
...
Produce a world-class, deeply researched high school profile for **Avenor College** in **Bucharest, Romania**, aligned to the global template and synced with the country schools dataset so recruiting teams can execute outreach with confidence.
...
- Priority tier / notes: Priority tier A per feeder map (school_id: sch-ro-bucharest-avenor)

---

## Allowed write paths

- `tickets/T-247-romania-sch-ro-bucharest-avenor-profile.md`
...
- `countries/romania/entities/schools/profiles/sch-ro-bucharest-avenor/README.md` created or refreshed using the global high school profile template with completed snapshot, affordability, curriculum, outcomes, outreach plan, and sources.
- `countries/romania/data/entities/schools/romania-high-potential-feeder-high-schools.csv` entry for `sch-ro-bucharest-avenor` aligned to the profile (tier, affordability signal, program-fit tags, sources).
- `countries/romania/data/entities/schools.csv` row for `sch-ro-bucharest-avenor` updated with city/region, curriculum, priority tier, affordability evidence, and last_verified tied to the source report.
...
- [ ] Data rows in `countries/romania/data/entities/schools.csv` (and any country-specific high-potential CSV) mirror the profile for `Avenor College`: slug, city, tier, curricula, affordability signals, program-fit tags, evidence/source references, and `last_verified` date are consistent.
...
- What changed (2025-12-21): Added full Avenor College profile, aligned CSV entries (schools.csv + high-potential feeder list), and updated Bucharest schools README.
