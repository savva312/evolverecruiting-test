...
Schema reference for `event-roi.csv`. Follow `countries/bulgaria/data/field-standards.md` for null handling, currency conventions, and date formats.

## Definitions (why this exists)

This dataset is a lightweight ROI tracker for fairs/events so the Bulgaria team can:
...
| --- | --- | --- | --- | --- |
| event_roi_id | Primary key for this ROI row (usually one row per event stop/date/city). | string | `world_education_fair_sofia_2026_spring` | Stable ASCII string. Do not recycle. |
| event_id | Foreign key to the canonical event entity record (planned in T-420 `fairs-events.csv`). | string | `wef_integral` | Optional until T-420 exists; leave blank if unknown. |
| event_name | Human-readable event name. | string | `World Education Fair (Integral)` | Use the organizer-facing name. |
| event_format | How the event was delivered. | enum | `in_person` | Suggested: `in_person`, `virtual`, `hybrid`. |
| event_city | City where the activity happened (blank for purely virtual). | string | `Sofia` | Free text. |
| country | ISO 3166-1 alpha-2 country code for the city/location. | string | `BG` | Use `BG` for Bulgaria. |
