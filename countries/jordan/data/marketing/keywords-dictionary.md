...
| audience | Target audience description. | string | `HS seniors in Amman` | Short segment description. |
| est_cpc_eur | Estimated cost-per-click in EUR. | decimal | `0.85` | Numeric only. |
| search_volume_jo | Average monthly searches in Jordan. | integer | `540` | Leave blank if unknown. |
| priority | Operational priority flag. | enum | `high` | Use `high`, `medium`, `low`, or blank. |
| notes | Source or targeting nuance. | string | `Based on Google KW Planner Jan 2026` | Concise. |
