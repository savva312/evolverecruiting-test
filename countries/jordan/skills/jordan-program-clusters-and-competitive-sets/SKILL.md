---
name: jordan-program-clusters-and-competitive-sets
description: Define and maintain Jordan-specific program clusters and competitive sets for recruiting, including competitor mapping and portfolio alignment.
metadata:
  owner: evobuilder
  version: "1.0"
  scope: jordan
...
# Jordan Program Clusters and Competitive Sets

This skill defines how EvoBuilder should organize Jordan recruitment intelligence by **program cluster** (e.g., Medicine vs CS/Data vs Business) and maintain a **Jordan-relevant competitive set** for each cluster.

It exists because Jordanian families do not “choose a university” in the abstract — they shortlist **by program**, and the competitor set for Medicine is completely different from the competitor set for CS/Data, Psychology, Business, etc.

---

## How this relates to the global skill

Use the global workflow in [`skills/program-clusters-and-competitor-sets/SKILL.md`](/skills/program-clusters-and-competitor-sets/SKILL.md) for definitions, steps, and templates. This Jordan file serves as a country addendum: keep Jordan-specific cluster taxonomy choices and any Jordan-only nuances here, and point tickets to the global skill for the end-to-end process.
...
- “Create competitor profiles for Jordan decision makers”
- “Update pricing + language-of-instruction tables by cluster”
- “Compare UNIC Athens vs UNIC Nicosia vs competitors (Jordan lens)”
...
- A similar Jordan buyer journey (student/parent motivations, timing, proof needed)
...
### Competitive set
A **competitive set** is the small list of institutions/programs Jordanian families *actually compare* against UNIC Athens / UNIC Nicosia for that cluster.

It should be:
- **Jordan-relevant** (realistic shortlists for Jordanian candidates)
...
## Recommended cluster set for Jordan recruiting repos

See the country addendum below for the Jordan-specific cluster taxonomy currently in use. Use the global baseline unless Jordan-specific demand signals require a different grouping.
...
- “Why it competes” (the reason Jordan families compare it)
- Source links
...
### Step 3 — Define Jordan decision logic for the cluster
Write (briefly, but concretely):

- Jordan student persona(s) for this cluster
...
- **Jordan domestic** options (when they truly compete)
- **Greece** (especially Athens/Thessaloniki) options
- **Cyprus** options
- **CEE** options (common for Medicine especially)
- **Western Europe** options (Germany/NL/etc.) when capacity/brand makes them appear on Jordanian shortlists
- **Private EU English-taught** institutions that Jordan families actually consider

For each competitor, record:
- why it competes for this cluster (one sentence)
- a source link for tuition + admissions mechanism

Avoid “random lists”. Every competitor must be justified by Jordan relevance.
...
### 2) Jordan decision journey (what matters)
- Personas:
- Proof required:
- Timing constraints:
- Objections:

### 3) Competitive set (Jordan-relevant)
#### A) Greece
- ...
#### B) Cyprus
- ...
#### C) CEE / Other EU
- ...
#### D) Jordan domestic (only if relevant)
- ...

### 4) Positioning and messaging (Jordan lens)
...
## Why it matters for Jordan (cluster-specific)
- Cluster(s):
- Why Jordanian students compare it:

## Programs that compete (Jordan-relevant only)
...
- A competitive set exists with justified, Jordan-relevant competitors
...
- Mixing “Europe recruiting” generalities into Jordan-specific artifacts
- Listing competitors without explaining why Jordanians compare them
- Writing strategy without updating CSVs (the repo becomes non-operational)
- Writing outside ticket boundaries

---

## Jordan addendum: cluster taxonomy and notes

Use this grouping when working in Jordan unless a ticket specifies otherwise:

1. **medicine-and-health**
   - Includes MD and any health/allied health programs marketed to Jordan.
2. **computing-data-ai**
...
5. **law-and-governance**
   - Law programs where Athens/Nicosia differences matter for Jordan prospects.
6. **design-and-creative**
   - Design and creative programs marketed to Jordan.
7. **other-strategic-programs**
   - Only when a Jordan demand signal and distinct competitor set are documented (e.g., Pharmacy if actively marketed).

If a proposed “cluster” would not change the competitive set or channel strategy for Jordan, keep it inside an existing cluster.
...
- Jordan relevance is the filter: if you can’t explain why it competes, omit it.
