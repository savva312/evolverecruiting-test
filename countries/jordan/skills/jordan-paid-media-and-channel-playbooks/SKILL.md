---
name: jordan-paid-media-and-channel-playbooks
description: Build and maintain Jordan-specific paid media + channel playbooks (Search/Meta/TikTok/YouTube/retargeting/messaging), tied to Jordan’s calendar and UNIC/UNIC Athens program clusters. Produces executable campaign structures, creative briefs/copy libraries, KPI guardrails, and structured CSVs for tracking spend, UTMs, CPL/CPA, and funnel performance.
metadata:
  owner: evobuilder
  version: "1.0"
  scope: jordan
...
# Jordan: paid media & channel playbooks (skill)
...
- Use this Jordan file for calendar, flighting, localization, consent, and compliance specifics that layer onto the global spec.

### Jordan addendum: localization and compliance anchors
- Language: AR + EN variants; prioritize AR hooks for upper-funnel and include subtitles where appropriate. Keep LPs bilingual when possible.
- Consent and messaging: WhatsApp/SMS only with explicit opt-in; avoid cold outreach. Respect local privacy expectations even when leveraging global GDPR baseline.
- Regulated programs: For Medicine/health, include recognition/licensure caveats and avoid employment or visa guarantees. Consider Ramadan timing and national exam periods when scheduling flights.

## Purpose
This skill turns Jordan recruiting strategy into **executable, measurable acquisition** across digital channels—without drifting into generic EU recruiting advice.

It produces:
- Jordan-specific channel strategy by **program cluster** (e.g., Medicine vs CS/Data/AI vs Business, etc.)
...
- “Jordan paid media plan” (budget, channels, flighting, KPIs)
- “Search keyword plan for Jordan” (Jordan + EN queries, negatives)
- “Meta/TikTok creative system + copy library (Jordanian)”
...
- Jordan education calendar + exam blackouts
- Priority schools + city clusters (Amman / Irbid / Zarqa / Aqaba + regional)
...
- Jordan-specific seasonality + flighting (Jan–Oct; exam blackouts)
- Creative system + hooks (by persona/cluster)
- Landing page requirements (Jordan-ready)
...
### C) Copy blocks (Jordanian + English)
...
### Step 1 — Anchor to Jordan’s calendar (non-negotiable)
Use the repo’s Jordan calendar skill outputs to define:
...
Deliverable: a short “flight map” table for Jordan.

### Step 2 — Define personas per cluster
At minimum:
- Student (high intent) + Parent (co-decision) + Counselor/Agent influence
- Jordan-specific variations (Amman IB school vs regional elite language school, etc.)
...
- Hooks must be Jordan-relevant (proximity, calendar deadlines, transparent cost)
...
Define the minimum LP elements to match Jordan expectations:
...
These are starting points; calibrate using historical Jordan results if available.
...
- Generic EU recruiting advice (not Jordan-specific)
...
- There is an executable Jordan channel plan tied to calendar windows
