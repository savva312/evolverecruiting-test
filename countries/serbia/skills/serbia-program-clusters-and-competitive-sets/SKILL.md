---
name: serbia-program-clusters-and-competitive-sets
description: Define and maintain Serbia-specific program clusters and competitive sets for recruiting, including competitor mapping and portfolio alignment.
metadata:
  owner: evobuilder
  version: "1.0"
  scope: serbia
...
# Serbia Program Clusters and Competitive Sets

This skill defines how EvoBuilder should organize Serbia recruitment intelligence by **program cluster** (e.g., Medicine vs CS/Data vs Business) and maintain a **Serbia-relevant competitive set** for each cluster.

It exists because Serbian families do not “choose a university” in the abstract — they shortlist **by program**, and the competitor set for Medicine is completely different from the competitor set for CS/Data, Psychology, Business, etc.

---

## How this relates to the global skill

Use the global workflow in [`skills/program-clusters-and-competitor-sets/SKILL.md`](/skills/program-clusters-and-competitor-sets/SKILL.md) for definitions, steps, and templates. This Serbia file serves as a country addendum: keep Serbia-specific cluster taxonomy choices and any Serbia-only nuances here, and point tickets to the global skill for the end-to-end process.
...
- “Create competitor profiles for Serbia decision makers”
- “Update pricing + language-of-instruction tables by cluster”
- “Compare UNIC Athens vs UNIC Nicosia vs competitors (Serbia lens)”
...
- A similar Serbia buyer journey (student/parent motivations, timing, proof needed)
...
### Competitive set
A **competitive set** is the small list of institutions/programs Serbian families *actually compare* against UNIC Athens / UNIC Nicosia for that cluster.

It should be:
- **Serbia-relevant** (realistic shortlists for Serbian candidates)
...
## Recommended cluster set for Serbia recruiting repos

See the country addendum below for the Serbia-specific cluster taxonomy currently in use. Use the global baseline unless Serbia-specific demand signals require a different grouping.
...
- “Why it competes” (the reason Serbian families compare it)
- Source links
...
### Step 3 — Define Serbia decision logic for the cluster
Write (briefly, but concretely):

- Serbia student persona(s) for this cluster
...
- **Serbia domestic** options (when they truly compete)
- **Greece** (especially Athens/Thessaloniki) options
- **Cyprus** options
- **CEE** options (common for Medicine especially)
- **Western Europe** options (Germany/NL/etc.) when capacity/brand makes them appear on Serbian shortlists
- **Private EU English-taught** institutions that Serbia families actually consider

For each competitor, record:
- why it competes for this cluster (one sentence)
- a source link for tuition + admissions mechanism

Avoid “random lists”. Every competitor must be justified by Serbia relevance.
...
### 2) Serbia decision journey (what matters)
- Personas:
- Proof required:
- Timing constraints:
- Objections:

### 3) Competitive set (Serbia-relevant)
#### A) Greece
- ...
#### B) Cyprus
- ...
#### C) CEE / Other EU
- ...
#### D) Serbia domestic (only if relevant)
- ...

### 4) Positioning and messaging (Serbia lens)
...
## Why it matters for Serbia (cluster-specific)
- Cluster(s):
- Why Serbian students compare it:

## Programs that compete (Serbia-relevant only)
...
- A competitive set exists with justified, Serbia-relevant competitors
...
- Mixing “Europe recruiting” generalities into Serbia-specific artifacts
- Listing competitors without explaining why Serbians compare them
- Writing strategy without updating CSVs (the repo becomes non-operational)
- Writing outside ticket boundaries

---

## Serbia addendum: cluster taxonomy and notes

Use this grouping when working in Serbia unless a ticket specifies otherwise:

1. **medicine-and-health**
   - Includes MD and any health/allied health programs marketed to Serbia.
2. **computing-data-ai**
...
5. **law-and-governance**
   - Law programs where Athens/Nicosia differences matter for Serbia prospects.
6. **design-and-creative**
   - Design and creative programs marketed to Serbia.
7. **other-strategic-programs**
   - Only when a Serbia demand signal and distinct competitor set are documented (e.g., Pharmacy if actively marketed).

If a proposed “cluster” would not change the competitive set or channel strategy for Serbia, keep it inside an existing cluster.
...
- Serbia relevance is the filter: if you can’t explain why it competes, omit it.
