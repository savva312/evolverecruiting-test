---
name: romania-program-clusters-and-competitive-sets
description: Define and maintain Romania-specific program clusters and competitive sets for recruiting, including competitor mapping and portfolio alignment.
metadata:
  owner: evobuilder
  version: "1.0"
  scope: romania
...
# Romania Program Clusters and Competitive Sets

This skill defines how EvoBuilder should organize Romania recruitment intelligence by **program cluster** (e.g., Medicine vs CS/Data vs Business) and maintain a **Romania-relevant competitive set** for each cluster.

It exists because Romanian families do not “choose a university” in the abstract — they shortlist **by program**, and the competitor set for Medicine is completely different from the competitor set for CS/Data, Psychology, Business, etc.

---

## How this relates to the global skill

Use the global workflow in [`skills/program-clusters-and-competitor-sets/SKILL.md`](/skills/program-clusters-and-competitor-sets/SKILL.md) for definitions, steps, and templates. This Romania file serves as a country addendum: keep Romania-specific cluster taxonomy choices and any Romania-only nuances here, and point tickets to the global skill for the end-to-end process.
...
- “Create competitor profiles for Romania decision makers”
- “Update pricing + language-of-instruction tables by cluster”
- “Compare UNIC Athens vs UNIC Nicosia vs competitors (Romania lens)”
...
- A similar Romania buyer journey (student/parent motivations, timing, proof needed)
...
### Competitive set
A **competitive set** is the small list of institutions/programs Romanian families *actually compare* against UNIC Athens / UNIC Nicosia for that cluster.

It should be:
- **Romania-relevant** (realistic shortlists for Romanian candidates)
...
## Recommended cluster set for Romania recruiting repos

See the country addendum below for the Romania-specific cluster taxonomy currently in use. Use the global baseline unless Romania-specific demand signals require a different grouping.
...
### Step 3 — Define Romania decision logic for the cluster
Write (briefly, but concretely):

- Romania student persona(s) for this cluster
...
- **Romania domestic** options (when they truly compete)
- **Greece** (especially Athens/Thessaloniki) options
- **Cyprus** options
- **CEE** options (common for Medicine especially)
- **Western Europe** options (Germany/NL/etc.) when capacity/brand makes them appear on Romanian shortlists
- **Private EU English-taught** institutions that Romania families actually consider

For each competitor, record:
- why it competes for this cluster (one sentence)
- a source link for tuition + admissions mechanism

Avoid “random lists”. Every competitor must be justified by Romania relevance.
...
### 2) Romania decision journey (what matters)
- Personas:
- Proof required:
- Timing constraints:
- Objections:

### 3) Competitive set (Romania-relevant)
#### A) Greece
- ...
#### B) Cyprus
- ...
#### C) CEE / Other EU
- ...
#### D) Romania domestic (only if relevant)
- ...

### 4) Positioning and messaging (Romania lens)
...
## Why it matters for Romania (cluster-specific)
- Cluster(s):
- Why Romanian students compare it:

## Programs that compete (Romania-relevant only)
...
- A competitive set exists with justified, Romania-relevant competitors
...
- Mixing “Europe recruiting” generalities into Romania-specific artifacts
- Listing competitors without explaining why Romanians compare them
- Writing strategy without updating CSVs (the repo becomes non-operational)
- Writing outside ticket boundaries

---

## Romania addendum: cluster taxonomy and notes

Use this grouping when working in Romania unless a ticket specifies otherwise:

1. **medicine-and-health**
   - Includes MD and any health/allied health programs marketed to Romania.
2. **computing-data-ai**
...
5. **law-and-governance**
   - Law programs where Athens/Nicosia differences matter for Romania prospects.
6. **design-and-creative**
   - Design and creative programs marketed to Romania.
7. **other-strategic-programs**
   - Only when a Romania demand signal and distinct competitor set are documented (e.g., Pharmacy if actively marketed).

If a proposed “cluster” would not change the competitive set or channel strategy for Romania, keep it inside an existing cluster.
...
- Romania relevance is the filter: if you can’t explain why it competes, omit it.
