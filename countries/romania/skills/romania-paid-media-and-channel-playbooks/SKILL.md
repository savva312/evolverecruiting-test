---
name: romania-paid-media-and-channel-playbooks
description: Build and maintain Romania-specific paid media + channel playbooks (Search/Meta/TikTok/YouTube/retargeting/messaging), tied to Romania’s calendar and UNIC/UNIC Athens program clusters. Produces executable campaign structures, creative briefs/copy libraries, KPI guardrails, and structured CSVs for tracking spend, UTMs, CPL/CPA, and funnel performance.
metadata:
  owner: evobuilder
  version: "1.0"
  scope: romania
...
# Romania: paid media & channel playbooks (skill)
...
- Use this Romania file for calendar, flighting, localization, and compliance specifics that layer onto the global spec.

### Romania addendum: localization and compliance anchors
- Language: RO + EN variants for ads and LPs; subtitles for video when RO primary. Keep tuition and recognition statements sourced.
- Consent: Assume GDPR/ANSPDCP requirements; ensure explicit opt-in for WhatsApp/Viber/SMS and clearly state purpose/retention.
- Regulated programs: For Medicine/health, include recognition/licensure disclaimers and avoid job/visa guarantees. Align timing with Bacalaureat exam windows when flighting.

## Purpose
This skill turns Romania recruiting strategy into **executable, measurable acquisition** across digital channels—without drifting into generic EU recruiting advice.

It produces:
- Romania-specific channel strategy by **program cluster** (e.g., Medicine vs CS/Data/AI vs Business, etc.)
...
- “Romania paid media plan” (budget, channels, flighting, KPIs)
- “Search keyword plan for Romania” (RO + EN queries, negatives)
- “Meta/TikTok creative system + copy library (Romanian)”
...
- Romania education calendar + exam blackouts
- Priority schools + city clusters (Bucharest / Cluj-Napoca / Constanta / Timisoara + regional)
...
- Romania-specific seasonality + flighting (Jan–Oct; exam blackouts)
- Creative system + hooks (by persona/cluster)
- Landing page requirements (Romania-ready)
...
### C) Copy blocks (Romanian + English)
...
### Step 1 — Anchor to Romania’s calendar (non-negotiable)
Use the repo’s Romania calendar skill outputs to define:
...
Deliverable: a short “flight map” table for Romania.

### Step 2 — Define personas per cluster
At minimum:
- Student (high intent) + Parent (co-decision) + Counselor/Agent influence
- Romania-specific variations (Bucharest IB school vs regional elite language school, etc.)
...
- Hooks must be Romania-relevant (proximity, calendar deadlines, transparent cost)
...
Define the minimum LP elements to match Romania expectations:
...
These are starting points; calibrate using historical Romania results if available.
...
- Generic EU recruiting advice (not Romania-specific)
...
- There is an executable Romania channel plan tied to calendar windows
