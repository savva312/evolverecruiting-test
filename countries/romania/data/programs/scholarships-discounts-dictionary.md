...
| currency | Currency code when `amount_type=fixed_amount`. | string (ISO 4217 or unit tag) | `EUR` | Use `EUR` for fixed-amount awards; leave blank for percentages, or use a unit tag such as `percent` only if consistent with other country datasets. |
