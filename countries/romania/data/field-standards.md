...
- **Multi-select lists:** Use semicolons with no spaces (`athens;online`) and order values alphabetically when practical, unless a dataset dictionary explicitly specifies pipe-delimited lists (`value1|value2`).

## Names, locations, and contact fields
- **Country codes:** ISO 3166-1 alpha-2 for `country` columns (e.g., `RO`, `CY`).
- **Regions/cities:** Free text; follow Romanian official spellings in English (e.g., `Bucharest`, `Cluj-Napoca`).
